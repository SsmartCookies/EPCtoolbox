...
[time,wPtot] = ScaleT2(pDate,pVars,vScale,'sum');
[~,wDtot] = ScaleT2(pDate,dVars,vScale,'sum');
...
[~,dPtot] = ScaleT2(pDate,pVars2,vScale,'sum');
[~,dDtot] = ScaleT2(pDate,dVars2,vScale,'sum');
Result.PRCPTOT.dPtot = dPtot;   Result.PRCPTOT.dDtot = dDtot;
%% total precipitation and number of different duration  wet and dry days  
pDate = Date; dVars = bsxfun(@ge,Vars,vWet);  pVars = Vars.*dVars;
[time,ConMaxWetD,ConWetD] = ContinuityDays(pDate,dVars,pVars,vScale);
Result.CWD.Date = time;   Result.CWD.MaxWD = ConMaxWetD;
Result.CWD.ConWD = ConWetD;
%------------------------------
dVars2 = bsxfun(@lt,Vars,vWet); pVars2 = Vars.*dVars;
[~,ConMaxDry,ConDdry] = ContinuityDays(pDate,dVars2,pVars2,vScale);
...
[time,Pxx] = ScaleT2(pDate,Vars.*dVars,vScale,'sum');
[~,Dxx] = ScaleT2(pDate,dVars,vScale,'sum');
...
[time,ExP] = ScaleT2(pDate,Vars.*dVars ,vScale,'sum'); 
[~,ExD] = ScaleT2(pDate,dVars,vScale,'sum');
...
function [OutDate,OutVars] = ScaleT2(Date,Vars,vScale,vMethod)
...
function [time,ConMax,ConXD] = ContinuityDays(Date,Vars1,Vars2,vScale)
if strcmpi(vScale,'d2m'),        nScale=2;
elseif strcmpi(vScale,'d2y'),    nScale=1;
end   
[time,~,class]  = unique(Date(:,1:nScale),'rows');
for i = 1:length(time)    
    ObjectD  = Date((class==i),:);
    ObjectV1 = Vars1((class==i),:);
    ObjectV2 = Vars2((class==i),:);
    for j=1:size(ObjectV1,2)
        temp = diff([0;ObjectV1(:,j);0]);  ids = find(temp==1);  ide = find(temp==-1); 
        ...
        for h=1:length(conD)
            ...
            IDset = [ids(Ds==conD(h)),ide(Ds==conD(h))-1];
            for k=1:size(IDset,1)
                ConXD{i,j}{h,4}(k,1)=sum(ObjectV2([IDset(k,1):IDset(k,2)]));
            end
            % special max data
            if h==length(conD)
            ConMax(i,j) = conD(h);
